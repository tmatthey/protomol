...
// Force test
  ...
  force_test = fopen("force_test","w");
  report << plain << "POSITIONS: " << endr;
    for (unsigned int i = 0; i < numatoms; i++)
    report << plain <<atomTypes[i]<<" "<< positions[i*3] << " " << positions[i*3+1] << " " << positions[i*3+2] << endr;
  myValues = scpenergy(numatoms, positions, charges, bonds, numbonds, angles, numangles, dihedrals, numdihedrals, atomTypes, rs,exclusion);
  for (unsigned int i = 0; i < numatoms; i++)
    report << plain <<atomTypes[i]<<" "<< myValues.force[i*3] << " " << myValues.force[i*3+1] << " " << myValues.force[i*3+2] << endr;
  double V0 = myValues.energy;
  fprintf(force_test,"-------Numerical Force Test: e = 1.e-5 ------------\n");
  double eps = 1.e-5;
  for (unsigned int i = 0; i < numatoms; i++) {
    positions[i*3] += eps;      
    myValues = scpenergy(numatoms, positions, charges, bonds, numbonds, angles, numangles, dihedrals, numdihedrals, atomTypes, rs,exclusion);
    double V = myValues.energy;
    double f_num = -(V-V0)/eps;
    positions[i*3] -= eps;
    myValues = scpenergy(numatoms, positions, charges, bonds, numbonds, angles, numangles, dihedrals, numdihedrals, atomTypes, rs,exclusion);
    double f_an  = myValues.force[i*3];
    fprintf(force_test,"atom %d, fx_num %.16g, fx_an %.16g\n", i+1, f_num, f_an);  
  }


  fprintf(force_test,"-------Numerical Force Test: e = 1.e-7 ------------\n");
  eps = 1.e-7;
  for (unsigned int i = 0; i < numatoms; i++) {
    positions[i*3] += eps;      
    myValues = scpenergy(numatoms, positions, charges, bonds, numbonds, angles, numangles, dihedrals, numdihedrals, atomTypes, rs,exclusion);
