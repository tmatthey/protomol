...
namespace ProtoMol {
  ...
  //_________________________________________________________________ CoulombBornRadiiForce
  ...
  public:
    void operator()(Real &energy, 
      ...
      //Real f_ij = f(distSquared);
      //Real fp_ij = fPrime(dist);
	  ...
	  //std::cout<<"A1 "<<atom1<<" , A2 "<<atom2<<" "<<std::endl;

      //**********************************
      // Eq. 6
      info[atom1].dRs[atom2] = gamma*B_i*dR_vdw2*(fp_ij*exp(-C_i*dist)-C_i*f_ij*exp(-C_i*dist));
      //dRs[atom1][atom2] = gamma*B_i*dR_vdw2*(fp_ij*exp(-C_i*dist)-C_i*f_ij*exp(-C_i*dist));
      //**********************************
	...
	if (topo->atoms[atom1].name == "HN" &&
 	...
 	topo->atoms[atom1].mySCPISM->polarFrac += g_i*g_j*f_ij*exp(-E_i*dist);
	//std::cout<<"Atom1 "<<atom1<<","<<topo->atoms[atom1].name<<", Atom2 "<<atom2<<", "<<topo->atoms[atom2].name<<" g_i "<<g_i<<" g_j "<<g_j<<std::endl;
	//**********************************
	// Eq. 8
	info[atom1].dRs[atom2] += g_i*g_j*(fp_ij*exp(-E_i*dist)+f_ij*exp(-E_i*dist)*-E_i);
	//dRs[atom1][atom2] += g_i*g_j*(fp_ij*exp(-E_i*dist)+f_ij*exp(-E_i*dist)*-E_i);
	//**********************************
	}
      ...
      //**********************************
      // Eq. 6
      info[atom2].dRs[atom1] = gamma*B_i*dR_vdw2*(fp_ij*exp(-C_i*dist)-C_i*f_ij*exp(-C_i*dist));
      //dRs[atom2][atom1] = gamma*B_i*dR_vdw2*(fp_ij*exp(-C_i*dist)-C_i*f_ij*exp(-C_i*dist));
      //**********************************
	 ...
	 if (topo->atoms[atom2].name == "HN" &&
	   ...
	   g_i = topo->atomTypes[type2].mySCPISM->g_i;
	 Real g_j = topo->atomTypes[type1].mySCPISM->g_i;	
	 topo->atoms[atom2].mySCPISM->polarFrac += g_i*g_j*f_ij*exp(-E_i*dist);
	//std::cout<<"Atom2 "<<atom2<<", "<<topo->atoms[atom2].name<<", Atom1 "<<atom1<<", "<<topo->atoms[atom1].name<<" g_i "<<g_i<<" g_j "<<g_j<<std::endl;
	 //**********************************
	 // Eq. 8
	 info[atom2].dRs[atom1] += g_i*g_j*(fp_ij*exp(-E_i*dist)+f_ij*exp(-E_i*dist)*-E_i);
	 //	 dRs[atom2][atom1] += g_i*g_j*(fp_ij*exp(-E_i*dist)+f_ij*exp(-E_i*dist)*-E_i);
	 //**********************************
      }
    ...
    static CoulombBornRadiiForce make(std::string&, const std::vector<Value>&); 
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // Sub Classes
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  public:
    ...
    public:
      ...
      static std::string getForceKeyword()            {return CoulombBornRadiiForce::keyword;}    
    public:
  ...
  public:
      ...
      Real BornSwitchDerivative(Real r) const {
        Real f,fr;
          if(sw==0) {
            ...
            fr = -9*.008*r*r*(1-.008*r*r*r)*(1-.008*r*r*r);
            return fr;
          }else {
            //sw==3
            f = (1-.0016*r*r*r*r);
   ...
   Real BornSwitchSecondDerivative(Real r) {
          ...
          if(sw==0) {
            ...
            //sw==3 function
            f = (1-.0016*r*r*r*r);
            frr = 0.00049152*f*f*r*r*r*r*r*r-0.0768*f*f*f*r*r;
            return frr;
          }else {
            f=(1-0.04*r*r);
