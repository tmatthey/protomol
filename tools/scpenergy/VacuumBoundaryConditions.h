...
namespace ProtoMol {
  ...
  public:
    ...
    Vector3D minimalDifference(const Vector3D &c1, const Vector3D &c2) const {Vector3D diff(c2); diff -= c1; return diff; }
    // Perform a minimal-image subtraction and computes the squared distance
    Vector3D minimalDifference(const Vector3D &c1, const Vector3D &c2, Real& distSquared) const{Vector3D diff(c2); diff -= c1;distSquared = diff.normSquared(); return diff;}
    ...
    Vector3D minimalTranslationDifference(const Vector3D &, const Vector3D &) const {return Vector3D(0.0,0.0,0.0); }
    /// Find the lattice translation relative to the original cell/image
    Vector3D minimalTranslationPosition(const Vector3D &) const {return Vector3D(0.0,0.0,0.0); }
    ...
    static VacuumBoundaryConditions make(std::string& errMsg, std::vector<Value> values);
    static unsigned int getParameterSize() {return 0;}
    std::vector<Parameter> getDefaults(const Vector3DBlock&) const{ return std::vector<Parameter>();}
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // My data members
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  public:
