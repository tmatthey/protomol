...
using namespace std;
namespace ProtoMol {
  //_________________________________________________________________ ExclusionPair
  ExclusionPair::ExclusionPair():a1(-1),a2(-1),excl(EXCLUSION_NONE){}
  ExclusionPair::ExclusionPair(int a,int b):a1(a),a2(b),excl(EXCLUSION_NONE){}
  ExclusionPair::ExclusionPair(int a,int b, ExclusionClass c):a1(a),a2(b),excl(c){}
  ...
  //_________________________________________________________________ ExclusionTable
  
  ExclusionTable::ExclusionTable():lowDeltas(NULL),anyExclsForThisDelta(NULL),myMaxDelta(0),myCurrentSize(0),myFastDeltaMax(0){}
  ...
  void ExclusionTable::optimize(){
	...
	for(int j=0;j<count;j++){
      ...
      delete [] lowDeltas;
      lowDeltas = tmp;
      report << hint << "Reduced fast delta max of ExclusionTable from "<<myFastDeltaMax-1<< " to "<<myMaxDelta<<"."<<endr;
      myFastDeltaMax = count;
    }
    else {
      report << hint << "Couldn't reduce fast delta max "<<myMaxDelta<<" of ExclusionTable ("<<myFastDeltaMax-1<<")."<<endr;
    }
  }
  ...
  void ExclusionTable::add(int atom1, int atom2, ExclusionClass type) {
    ...
    if(type != EXCLUSION_NONE && atom1 != atom2 && mySet.find(ExclusionPair(atom1,atom2,type)) == mySet.end()){
