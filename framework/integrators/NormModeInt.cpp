...
namespace ProtoMol {
  ...
  NormModeInt::NormModeInt() : MTSIntegrator(), fixMod(-1), myGamma(-1), mySeed(-1), myTemp(-1), myNVE(0), myBerendsen(0), fDof(0)
		{ex0=NULL; tmpFX=NULL; tmpC=NULL; invSqrtMass=NULL; sqrtMass=NULL;}
  ...
  Vector3DBlock* NormModeInt::subspaceForce(Vector3DBlock * force, Vector3DBlock * iPforce){
	...
	int m = _3N; int n = _rfM-fDof; int incxy = 1;	//sizes
  ...
  void NormModeInt::run(int numTimesteps) {
    ...
    for( int i = 0; i < numTimesteps; i++ ) {
	  ...
	  //
	  if(!myNVE && !myBerendsen){	//constant temp/Berendsen or langevin?
		drift();
	  }else{		//or energy?
	  ...
	  if(mhQ != NULL){	//not rediagonalize?
		  ...
		  //**************************************************
		  //
		  if(myBerendsen){	//weak Berendsen thermostat?
