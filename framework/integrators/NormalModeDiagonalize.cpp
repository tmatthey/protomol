...
namespace ProtoMol
{
	...
	void NormalModeDiagonalize::initialize(GenericTopology* topo,
	...
	void NormalModeDiagonalize::run(int numTimesteps)
	{
		...
		for (int i = 0; i < numTimesteps; i++)
		{
			...
			if ((rediagCount && (int)(myTopo->time / getTimestep()) >= nextRediag) || firstDiag)
			{
				...
				if (removeRand) (*myPositions).intoSubtract(myLastNormalMode->gaussRandCoord1);
				//minimizer AND valid maximum eigenvalue
				if (minSteps > 0 && validMaxEigv)
					minimizer(minLim, minSteps, true, false, true, &forceCalc, &lastLambda, myEnergies, myPositions, myTopo);
				//
				//Avergaged?
				...
				if (noAvStep > 1)
				{
					myAveragePos = *myPositions;
					numAveragePos = 1;
					Vector3DBlock tmpVel = *myVelocities; //save velocities
					VerletAverage();
					*myVelocities = tmpVel;
				...
				{
					hsn.evaluate(myPositions, myTopo, true);
				}
				...
				myNextNormalMode->subSpaceSift(myVelocities, myForces);
			}
			//run integrator
	...
	void NormalModeDiagonalize::removeBondEig()
	{
		...
		for (unsigned int i = 0; i < myTopo->bonds.size(); i++)
		{
			...
			if ((myTopo->atoms[a1].name.c_str())[0] == 'H' || (myTopo->atoms[a2].name.c_str())[0] == 'H')
			{
				//report << hint << "[NormalModeDiagonalize] i= "<<i<<" a1= "<<a1<<" a2 = "<<a2<<" name1= "<<app->topology->atoms[a1].name<<" name2= "<<app->topology->atoms[a2].name<<endr;
				aDiff = (*myPositions)[a2] - (*myPositions)[a1];
				//find ratio of masses
				Real rMass = myTopo->atoms[a1].scaledMass / myTopo->atoms[a2].scaledMass;
				...
				for (int j = 0; j < _idM; j++)
				{
					...
					//find dot product
					dotProd = 0.0;
					dotProd += mhQu[offs1 + 0] * aDiff.x * norm1;
					dotProd -= mhQu[offs2 + 0] * aDiff.x * norm2;
					dotProd += mhQu[offs1 + 1] * aDiff.y * norm1;
					dotProd -= mhQu[offs2 + 1] * aDiff.y * norm2;
					dotProd += mhQu[offs1 + 2] * aDiff.z * norm1;
					dotProd -= mhQu[offs2 + 2] * aDiff.z * norm2;
					//remove dotprodct times eigvectors
					mhQu[offs1 + 0] -= dotProd * aDiff.x * norm1;
					mhQu[offs2 + 0] += dotProd * aDiff.x * norm2;
					mhQu[offs1 + 1] -= dotProd * aDiff.y * norm1;
					mhQu[offs2 + 1] += dotProd * aDiff.y * norm2;
					mhQu[offs1 + 2] -= dotProd * aDiff.z * norm1;
					mhQu[offs2 + 2] += dotProd * aDiff.z * norm2;
	...
	void NormalModeDiagonalize::VerletAverage()
	{
		...
		for (int nos = 1; nos < noAvStep; nos++)
		{
			...
			for (int j = 0; j < _N; j++) myAveragePos[j] += (*myPositions)[j];
			numAveragePos++;
			//hsn.evaluate(myPositions, myTopo, true);	//true for mass re-weight;
			calculateForces();
			for (int i = 0; i < _N; ++i)
	...
	MTSIntegrator* NormalModeDiagonalize::doMake(string&, const vector<Value>& values, ForceGroup* fg, StandardIntegrator* nextIntegrator) const
	{
