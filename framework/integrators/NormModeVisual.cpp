...
namespace ProtoMol
{
	...
	NormModeVisual::NormModeVisual() : STSIntegrator()
	{
		ex0 = NULL;
		eigvalp = NULL;
		tmpFX = NULL;
	                                ...
	                                ScalarStructure* energies)
	{
		...
		*ex0 = *myPositions;
		...
		if (secondPosFile != "")
		{
			...
			else if (!(reader >> *secondPos))
				report << error << "Could not parse position file \'" << secondPosFile
			...
			if ((*secondPos).size() == (unsigned int)_N)
			{ //same size?
