...
namespace ProtoMol {
  ...
  {
	  sz = szin;
	  if(hessM==NULL) hessM = new double[sz*sz]; //assign array
  }
  ...
  void Hessian::findForces(ForceGroup *overloadedForces)
  {
	...
	//report <<hint<<"[LeapfrogIntCrs::initialize] myCoulumb="<<myCoulomb<<", myCoulombDielec="<<myCoulombDielec<<", myLJ="<<myLennardJones<<", myBond="<<myBond<<", myAngle="<<myAngle<<", nyDihedral="<<myDihedral<<", myImproper="<<myImproper<<endr;
	//report <<hint<<"[LeapfrogIntCrs::initialize] myCoulombDielec="<<myCoulombDielec<<", D="<<D<<", S="<<S<<", epsi="<<epsi<<endr;
  }

  void Hessian::evaluate(const Vector3DBlock *myPositions, const GenericTopology *myTopo, bool mrw)
  {
   ...
   if(myImproper){
		...
		for (unsigned int i = 0; i < myTopo->impropers.size(); i++){
			...
			if(nonZForce){
				//report <<hint<<"[LeapfrogIntCrs::initialize] impropers= "<<i<<" multi= "<<myTopo->impropers[i].multiplicity<<" per="<<myTopo->impropers[i].periodicity[0]<<endr;
				int aout[4];
				aout[0] = myTopo->impropers[i].atom1; aout[1] = myTopo->impropers[i].atom2;
				aout[2] = myTopo->impropers[i].atom3; aout[3] = myTopo->impropers[i].atom4;
				//
				hi.evaluate(myTopo->impropers[i],(*myPositions)[aout[0]],(*myPositions)[aout[1]],(*myPositions)[aout[2]],(*myPositions)[aout[3]]);
				//output sparse matrix
				for (int ii = 0; ii < 4; ii++){
					for(int kk = 0; kk < 4; kk++){
						int mi = ii * 36 + kk * 3;//kk * 36 + ii*3;
						Matrix3by3 rhd(hi.hessD[mi],hi.hessD[mi+1],hi.hessD[mi+2], 
										hi.hessD[mi+12],hi.hessD[mi+13],hi.hessD[mi+14], 
											hi.hessD[mi+24],hi.hessD[mi+25],hi.hessD[mi+26]); 
						for (int ll = 0; ll < 3; ll++){
							for(int mm = 0; mm < 3; mm++){
								if(mrw) hessM[(aout[ii]*3+ll)*sz+aout[kk]*3+mm] += 
											((rhd(ll,mm)/sqrt(myTopo->atoms[aout[ii]].scaledMass*myTopo->atoms[aout[kk]].scaledMass)));
								else hessM[(aout[ii]*3+ll)*sz+aout[kk]*3+mm] += rhd(ll,mm);
							}
						}
					}
				}
	...
	if(myDihedral){
		...
		for (unsigned int i = 0; i < myTopo->dihedrals.size(); i++){
			...
			if(nonZForce){
				//report <<hint<<"[LeapfrogIntCrs::initialize] dihedral= "<<i<<" multi= "<<myTopo->dihedrals[i].multiplicity<<" per="<<myTopo->dihedrals[i].periodicity[0]<<endr;
				int aout[4];
				aout[0] = myTopo->dihedrals[i].atom1; aout[1] = myTopo->dihedrals[i].atom2;
				aout[2] = myTopo->dihedrals[i].atom3; aout[3] = myTopo->dihedrals[i].atom4;
				//
				hd.evaluate(myTopo->dihedrals[i],(*myPositions)[aout[0]],(*myPositions)[aout[1]],(*myPositions)[aout[2]],(*myPositions)[aout[3]]);
				//output sparse matrix
				for (int ii = 0; ii < 4; ii++){
					for(int kk = 0; kk < 4; kk++){
						int mi = ii * 36 + kk * 3;//kk * 36 + ii*3;
						Matrix3by3 rhd(hd.hessD[mi],hd.hessD[mi+1],hd.hessD[mi+2], 
										hd.hessD[mi+12],hd.hessD[mi+13],hd.hessD[mi+14], 
											hd.hessD[mi+24],hd.hessD[mi+25],hd.hessD[mi+26]); 
						for (int ll = 0; ll < 3; ll++){
							for(int mm = 0; mm < 3; mm++){
								if(mrw) hessM[(aout[ii]*3+ll)*sz+aout[kk]*3+mm] += 
											((rhd(ll,mm) /sqrt(myTopo->atoms[aout[ii]].scaledMass*myTopo->atoms[aout[kk]].scaledMass)));
   ...
   if(myBond){
	for (i = 0; i < myTopo->bonds.size(); i++){
		...
		Matrix3by3 bondHess12 = reducedHessBond((*myPositions)[a1],(*myPositions)[a2],k,r_0);
		//output sparse matrix
		for (int ll = 0; ll < 3; ll++){
			for(int mm = 0; mm < 3; mm++){
				...
				if(mrw){
					ms1 = tempf/sqrt(myTopo->atoms[a1].scaledMass*myTopo->atoms[a1].scaledMass);
					ms2 = tempf/sqrt(myTopo->atoms[a2].scaledMass*myTopo->atoms[a2].scaledMass);
					ms3 = -tempf/sqrt(myTopo->atoms[a1].scaledMass*myTopo->atoms[a2].scaledMass);
				}else{
					ms2 = ms1 = tempf;
					ms3 = -tempf;
				}
		...
		for (int ii = 0; ii < 3; ii++){
		  for(int kk = 0; kk < 3; kk++){
		      ...
		      for (int ll = 0; ll < 3; ll++){
				  for(int mm = 0; mm < 3; mm++){
					if(mrw) hessM[(aout[ii]*3+ll)*sz+aout[kk]*3+mm] += 
								((rha(ll,mm)/sqrt(myTopo->atoms[aout[ii]].scaledMass*myTopo->atoms[aout[kk]].scaledMass)));
					else hessM[(aout[ii]*3+ll)*sz+aout[kk]*3+mm] += rha(ll,mm);
				  }
		      }
		  }
		}
			...
			if(ec != EXCLUSION_FULL){
				...
				for (int ll = 0; ll < 3; ll++){
                  for(int mm = 0; mm < 3; mm++){
						...
						tempf = rha(ll,mm);
						if(mrw){
							ms1 = tempf/sqrt(myTopo->atoms[i].scaledMass*myTopo->atoms[i].scaledMass);
							ms2 = tempf/sqrt(myTopo->atoms[j].scaledMass*myTopo->atoms[j].scaledMass);
							ms3 = -tempf/sqrt(myTopo->atoms[i].scaledMass*myTopo->atoms[j].scaledMass);
						}else{
							ms2 = ms1 = tempf;
							ms3 = -tempf;
						}
				...
				//report <<hint<<"[HessianInt::initialize] excluded LJ i="<<i<<", j="<<j<<endr;
			}
		}
	}
   }
	...
	for (unsigned int i = 0; i < myTopo->atoms.size(); i++){
		for (unsigned int j = i+1; j < myTopo->atoms.size(); j++){
			...
			if(ec != EXCLUSION_FULL){
				...
				//report <<hint<<"[HessianInt::initialize] excluded Co i="<<i<<", j="<<j<<endr;
			}
		}
	}
   }
   ...
   if(myCoulombBornRadii) {
    ...
    void Hessian::evaluateCoulombSCPISM(const Vector3DBlock *myPositions, const GenericTopology *myTopo, bool mrw) {
       int eye, jay;
   Real tempf, ms1, ms2, ms3;
    Matrix3by3 rha;
    ExclusionClass ec;
    ...
    void Hessian::evaluateCoulombBornRadii(const Vector3DBlock *myPositions, const GenericTopology *myTopo, bool mrw) {
       int eye, jay;
       Real tempf, ms1, ms2, ms3;
       Matrix3by3 rha,rhc;
       ExclusionClass ec;
