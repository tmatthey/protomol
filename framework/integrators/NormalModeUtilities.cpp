...
namespace ProtoMol
{
	...
	int NormalModeUtilities::diagHessian(double* eigVecO, double* eigValO, double* hsnhessM, int dim, int& numFound)
	{
		...
		int m;
		int ldz = dim;
		int lwork = 26 * dim; /* dimension of work array*/
	...
	void NormalModeUtilities::absSort(double* eigVec, double* eigVal, int* eigIndx, int dim)
	{
		...
		for (i = 0; i < dim; i++)
		{
			if (eigIndx[i] != (int)i && eigIndx[i] != -1)
			{ //need to swap?
				for (int j = 0; j < dim; j++)
				{
		...
		*forceCalc = 0;
		...
		for (in = 0; in < numloop; in++)
		{
			...
			*lastLambda += lambda;
			...
			{
				if (rsCG > 4) report << error << "[NormalModeUtilities::minimizer] Minimization failed, Aborting." << endr;
				else
				{
					if (!reDiag)
