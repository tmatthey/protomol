...
namespace ProtoMol
{
	...
	CGMinimizer::CGMinimizer() : STSIntegrator()
	{
		gk = NULL;
		pk = NULL;
		oldPos = NULL;
	...
	CGMinimizer::CGMinimizer(Real timestep, Real alpha, Real beta2, Real restart, bool massweight,
		...
		: STSIntegrator(timestep, overloadedForces), myAlpha(alpha), myBeta(beta2), myRestart(restart), massWeight(massweight)
	{
		gk = NULL;
		pk = NULL;
		oldPos = NULL;
	...
	void CGMinimizer::run(int numTimesteps)
	{
		...
		for (int j = 0; j < numTimesteps; j++)
		{
			...
			*oldPos = *myPositions;
			...
			while (gpr > myBeta);
			//
			if (k > 8 || dU > (myAlpha * lambda * gp) || (myRestart > 0 && mcount > (int)myRestart))
