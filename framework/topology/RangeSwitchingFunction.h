...
namespace ProtoMol {
  ...
  public:
    ...
    enum {USE=1};
    enum {MODIFY=1};
    enum {CUTOFF=TOriginalSwitchingFunction::CUTOFF};
  ...
  public:
    ...
    Real cutoffSquared() const{return std::max(myR1Squared,myOrigFunc.cutoffSquared());}
    Real cutoff() const{return std::max(myR1,myOrigFunc.cutoff());}
    ...
    static unsigned int getParameterSize() {return 2+TOriginalSwitchingFunction::getParameterSize();}
