...
namespace ProtoMol {
  ...
  //_________________________________________________________________ CellListEnumerator_periodicBoundaries
  ...
  public:
    ...
    void initialize(const Topology<PeriodicBoundaryConditions,CubicCellManager>* topo,
		    Real cutoff) {
	...
	for(int k =-n0;k<=n0;k++){
	  ...
	  for(int l =-n1;l<=n1;l++){
		...
		CubicCellManager::Cell minimalDelta = myCellListStruct->basisCell(delta);
	...
	if(i!=the_end){
	  ...
	  while(the_counter != the_last && 
		the_end == (j=myCellListStruct->findPeriodic(i->first+myDeltaList[the_counter++])));
