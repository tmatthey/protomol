...
namespace ProtoMol
{
	...
	void CubicCellManager::initialize(CellListStructure& cellList, const Vector3D& min, const Vector3D& max, bool pbc) const
	{
		if (pbc)
		{
			Vector3D d(max - min);
	...
	{
		parameters.push_back(Parameter("cellSize", Value(myCellSize, ConstraintValueType::Positive()), Text("For Periodic BC this must be < least cell basis vector.  Typically 1/2 of the least cutoff value")));
	}

	CubicCellManager CubicCellManager::make(string& errMsg, vector<Value> values)
	{
		Real r = 1.0;
		if (!values[0].get(r) || r <= 0.0)
		{
			errMsg += keyword + " cellmanager: cutoff > 0 (" + values[0].getString() + ").";
