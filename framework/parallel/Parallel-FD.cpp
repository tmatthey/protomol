//  This is the force decomposition implementation by T. Matthey.  In order to
//  use this method, you will need to replace the file Parallel.h with this
//  file.  --ssh 2006-03-10
...
namespace ProtoMol {
  ...
  MPI_Comm  commX  = MPI_COMM_NULL;
  MPI_Comm  commY  = MPI_COMM_NULL;
  bool force = false;
  bool first = true;
  int*    recvcounts = NULL;
  int*    displs     = NULL;
  ...
  void allReduce(T* begin, T* end){
    ...
    if(dobarrier)
      doBarrier<exludeMaster>();
    MPI_Allreduce(&(tmp[0]), begin, (end-begin), MPITypeTraits<T>::datatype, MPI_SUM, (exludeMaster ? slaveComm:MPI_COMM_WORLD));  
  }
  
  template<bool exludeMaster, bool dobarrier>
  void allReduce(Vector3DBlock* coords){
    allReduce<exludeMaster,dobarrier>(&(coords->begin()->x),&(coords->end()->x));
  }
  ...
  void broadcast(T* begin, T* end){
    if(dobarrier)
      doBarrier<exludeMaster>();
    MPI_Bcast(begin, (end-begin), MPITypeTraits<T>::datatype, (exludeMaster ?0:master), (exludeMaster ? slaveComm:MPI_COMM_WORLD));  
  }
  
  template<bool exludeMaster, bool dobarrier>
  void broadcast(Vector3DBlock* coords){
    broadcast<exludeMaster, dobarrier>(&(coords->begin()->x),&(coords->end()->x));
  }

  template<bool exludeMaster, bool dobarrier>
  void broadcast(ScalarStructure* energies){
    broadcast<exludeMaster, dobarrier>(&((*energies)[ScalarStructure::FIRST]),&((*energies)[ScalarStructure::LASTREDUCE]));
  }
#endif 
...
//          std::cerr <<"Either you forgotten to call Parallel::finalize() or I got killed. Can't call it for you ... I'm already dead!"<<std::endl;
  ...
  void Parallel::setMode(ParallelType mode){
    ...
    FFTComplex::FFTComplexMPIInit(mode == ParallelType::MASTERSLAVE?myMasterId:-1);
#endif
...
// Overwrites the Vector3D with a new one after sending it to another node
...
#ifdef HAVE_MPI
  void Parallel::sendrecv(Real *senddata, int sendnum, int sendaddr, Real *recvdata, int recvnum, int recvaddr) {
    MPI_Status status;
...
#ifdef HAVE_MPI
  void Parallel::sendrecv_replace(Real *data, int num, int sendaddr, int recvaddr) {
    MPI_Status status;
  ...
  void Parallel::distribute(ScalarStructure* energies, Vector3DBlock* coords) {
    ...
    if(first){
      int d = (int)( -0.5 + sqrt( 1.0 + 8.0 * myNum ) / 2.0 );
      force = (myNum == ((d*(d+1))/2));
      if(force){
        vector<vector<int> > matrix(d,vector<int>(d,-1));
        recvcounts =   new int[d];
        displs     =   new int[d];
        for(int i=0;i<d;i++){
          int i0 = (coords->size()*i)/d;
          int i1 = (coords->size()*(i+1))/d;
          recvcounts[i] = (i1-i0)*3;
          displs[i]     = i0*3;
        }
        int p =0;
        for(int i=0;i<d;++i){
        ...
        for(int w=0;w<d;++w){
          // Create intracommunicator X
          vector<int> excl;
          for(int i=0;i<d;++i){
            for(int j=i;j<d;++j){
              if(matrix[i][j] >= 0 && i != w)
                excl.push_back(matrix[i][j]);
            }
          }
          if(excl.size() < myNum-1){
            MPI_Comm  tmp  = MPI_COMM_NULL;
            MPI_Group worldGroup = MPI_GROUP_NULL;
            MPI_Group localGroup = MPI_GROUP_NULL;
            MPI_Comm_group(MPI_COMM_WORLD,&worldGroup);
            MPI_Group_excl(worldGroup,excl.size(),&(excl[0]),&localGroup);
            if(w == x){
              MPI_Comm_create(MPI_COMM_WORLD,localGroup,&commX);
            }
            else{
              MPI_Comm_create(MPI_COMM_WORLD,localGroup,&tmp);
            }
            MPI_Group_free(&worldGroup);
            MPI_Group_free(&localGroup);
          }

          // Create intracommunicator Y
          excl.clear();
          for(int i=0;i<d;++i){
            for(int j=i;j<d;++j){
              if(matrix[i][j] >= 0 && j != w)
          ...
          if(excl.size() < myNum-1){
            ...
            MPI_Group_excl(worldGroup,excl.size(),&(excl[0]),&localGroup);
            if(w == y){
              MPI_Comm_create(MPI_COMM_WORLD,localGroup,&commY);
            }
            else {
              MPI_Comm_create(MPI_COMM_WORLD,localGroup,&tmp);
            }
        ...
        report << allnodesserial
               << "Using Force decompostions reduction (" << d << "," << myNum
               << "," << x << " ("
               << ( commX != MPI_COMM_NULL ? displs[x] / 3 : -1 ) << ","
               << ( commX != MPI_COMM_NULL ? ( displs[x] + recvcounts[x] ) / 3 : -1 )
               << ")" << "," << y << " ("
               << ( commY != MPI_COMM_NULL ? displs[y] / 3 : -1 )
               << ","
               << ( commY != MPI_COMM_NULL ? ( displs[y] + recvcounts[y] ) / 3 : -1 )
               << ")" << "," << ( commX != MPI_COMM_NULL ) << ","
               << ( commY != MPI_COMM_NULL ) << ")" << endr;
      }
    }
    first = false;
    energies->distribute();
  ...
  void Parallel::reduce(ScalarStructure* energies, Vector3DBlock* coords){
    ...
    if(force){
      coords->zero();
      vector<Real> tmp(&(coords->begin()->x),&(coords->end()->x));
      if(commX != MPI_COMM_NULL)
        MPI_Allreduce(&(tmp[0])+displs[x], &(coords->begin()->x)+displs[x], recvcounts[x], MPI_DOUBLE, MPI_SUM, commX); 
      if(commY != MPI_COMM_NULL)
        MPI_Allreduce(&(tmp[0])+displs[y], &(coords->begin()->x)+displs[y], recvcounts[y], MPI_DOUBLE, MPI_SUM, commY); 
      coords->zero();
    }
    else {
...
//     return  static_cast<unsigned int>(getAvailableNum());
...
#ifdef DEBUG_PARALLEL
      report << allnodes << plain << "Block["<<myI<<"] = ["<<myBlockList[myI]<<","<<myBlockList[myI+1]<<"] to "<<myP<<"."<<endr;
#endif
  ...
  bool Parallel::next(){
    ...
    case MASTER:
      ...
      while(myBlockList[myI+2]<0 && myBlockList[myI+0] >= 0 && myBlockList[myI+1] >=0);
...
#if defined(DEBUG_PARALLEL)
        report << allnodes << plain << "Block["<<myI<<"] = ["<<myBlockList[myI]<<","<<myBlockList[myI+1]<<"] to "<<getId()<<"."<<endr;
#endif
        myI++;
      }
    ...
    case Parallel::SLAVE:
      if(myNextRange[1] == myNext){
        ...
        TimerStatistic::timer[TimerStatistic::IDLE].start();
#ifdef DEBUG_PARALLEL
        report << allnodes << plain << "Recv new block "<<getId()<<"."<<endr;
#endif
        MPI_Recv(myNextRange,2,MPI_INT,myMasterId,SEND_RANGE,MPI_COMM_WORLD,&status);
#ifdef DEBUG_PARALLEL
...
#ifdef DEBUG_PARALLEL
          report << allnodes << plain << "Ask new block "<<getId()<<"."<<endr;
#endif
        }
        if(myNextRange[1] < 0)
          myWorkState = DONE;
