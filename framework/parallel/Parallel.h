...
namespace ProtoMol
{
	...
	//________________________________________________________ Parallel
	class Parallel
	{
	...
	public:
		...
		static bool ok()
		{
			return (myInitialized && !myFinalized);
		}
		...
		{
			return myAvailableId;
		}

		static int getAvailableNum()
		{
			return myAvailableNum;
		}
		...
		static bool iAmSlave()
		{
			return myIAmSlave;
		}

		static ParallelType getMode()
		{
			return myMode;
		}
		...
		{
			return (myMode != ParallelType::STATIC);
		}

		static int getPipeSize()
		{
			return myPipeSize;
		}
		...
		static void setBarrier(bool b)
		{
			myUseBarrier = b;
		}

		static int getMaxPackages()
		{
			return myMaxPackages;
		}
	...
	public:
		...
		static void sendrecv_replace(Vector3DBlock* Vect, int sendaddr, int recvaddr);
		...
		static void sendrecv(Real* senddata, int sendnum, int sendaddr, Real* recvdata, int recvnum, int recvaddr);
		/// Sends a Real (or array of Reals) to one node and overwrites it with data from another node.
		static void sendrecv_replace(Real* data, int num, int sendaddr, int recvaddr);
