...
namespace ProtoMol
{
	//_________________________________________________________________ OneAtomPairFull
	...
	public:
		...
		OneAtomPairFull(TNonbondedForce nF,
	...
	public:
		...
		static OneAtomPairFull make(std::string& errMsg, std::vector<Value> values)
		{
			unsigned int n = TNonbondedForce::getParameterSize();
			return OneAtomPairFull(TNonbondedForce::make(errMsg, std::vector<Value>(values.begin(), values.begin() + n)),
	...
	template <class TBoundaryConditions,
	          ...
	          class TNonbondedForce,
	          class TConstraint>
	inline void OneAtomPairFull<TBoundaryConditions,
	                            TSwitchingFunction,
	                            TNonbondedForce,
	                            TConstraint>::doOneAtomPair(const int i, const int j)
	{
		...
		if (!same)
		{
			...
			if (excl != EXCLUSION_FULL)
			{
				...
				nonbondedForceFunction(rawEnergy, rawForce, distSquared, rDistSquared, diffMinimal, realTopo, i, j, excl);
		...
		for (unsigned int k = 0; k < lattice->size(); k++)
		{
			...
			Real rawEnergy, rawForce;
			Real rDistSquared = 1.0 / distSquared;
			nonbondedForceFunction(rawEnergy, rawForce, distSquared, rDistSquared, diff, realTopo, i, j, EXCLUSION_NONE);
			// Calculate the switched force and energy.
			Real energy, force;
			if (TSwitchingFunction::USE)
			{
