...
namespace ProtoMol
{
	...
	template <class TBoundaryConditions,
	          ...
	          bool correction,
	          bool smooth>
	class NonbondedMultiGridSystemForce: public SystemForce, private NonbondedMultiGridSystemForceBase
	{
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Typedef
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	public:
		...
		{
			return (1 + (TBoundaryConditions::PERIODIC && smooth ? 3 : 0) + (smooth ? 3 : 0) + (TBoundaryConditions::VACUUM && smooth ? 2 : 0));
		}
	...
	Force* NonbondedMultiGridSystemForce<TBoundaryConditions,
	                                     ...
	                                     direct,
	                                     correction,
	                                     smooth>::doMake(std::string& errMsg, std::vector<Value> values) const
	{
		int i = 0;
		std::string err = "";
		Real s = values[i++];
			...
			if (levels <= 0)
			{
				...
				if (TBoundaryConditions::VACUUM)
				{
					if (h.x <= 0.0 || h.y <= 0.0 || h.z <= 0.0)
						err += " hx (=" + toString(h.x) + ") > 0, hy (=" + toString(h.y) + ")) > 0, hz (=" + toString(h.z) + ") > 0..";
				}
			}
		}
