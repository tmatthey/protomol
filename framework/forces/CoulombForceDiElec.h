...
namespace ProtoMol
{
	...
	public:
		...
		// Set X in DiElecCom = (D-X)*0.5 to your chosen value 2,3,4,etc.. which yields eps(0)=2,3,4,etc...
		// eps(0)=1 is insufficient to model solvent polarization effects between near molecules
		// Try 2,3,or 4
	...
	public:
		                ...
		                ExclusionClass excl) const
		{
			...
			Real drDistrDiElecPoly = S * S * Dist * Dist * Dist + 2 * S * Dist * Dist + 2 * Dist;
			Real drDistrDiElecP1 = DiElecCom * rDiElecEXP * (3 * S * S * Dist * Dist + 4 * S * Dist + 2);
			Real drDistrDiElecP2 = DiElecCom * rDiElecEXP * S * drDistrDiElecPoly;
			Real drDistrDiElecP3 = D * Dist - DiElecCom * rDiElecEXP * drDistrDiElecPoly;

			Real drDistrDiElec = (D - drDistrDiElecP1 + drDistrDiElecP2) / (drDistrDiElecP3 * drDistrDiElecP3);
			force = energy / rDiElec * drDistrDiElec;
			/**      
			if ((energy > 1000) || (energy < -1000) ||  (force > 1000) || (force < -1000))
		  {
			std::cout << "The energy and forces are becoming unstable" << std::endl;
			std::cout << "Energy: " << energy << "   ,Force: " << force << std::endl;
			std::cout << "For atom1: " << atom1 << " and atom2: " << atom2 << std::endl;
			std::cout << "Dist: " << Dist << std::endl;
			std::cout << "rDiElec: " << rDiElec << std::endl;
			std::cout << "drDistrDiElec: " << drDistrDiElec << std::endl;
			std::cout << "rDiElecEXP: " << rDiElecEXP << std::endl;
			std::cout << "drDistrDiElecPoly: " << drDistrDiElecPoly << std::endl;
			std::cout << "drDistrDiElecP1: " << drDistrDiElecP1 << std::endl;
			std::cout << "drDistrDiElecP2: " << drDistrDiElecP2 << std::endl;
			std::cout << "drDistrDiElecP3: " << drDistrDiElecP3 << std::endl;
			std::cout << std::endl;
		  }
			**/
			// no negative term - magnitude only
			// additional rDist term normalized force for later mult by directional vec components
		}
		...
		static CoulombForceDiElec make(std::string&, const std::vector<Value>&);
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Sub Classes
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	public:
		...
		public:
			...
			{
				return CoulombForceDiElec::keyword;
			}
