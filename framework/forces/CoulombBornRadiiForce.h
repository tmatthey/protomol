...
namespace ProtoMol
{
	...
	public:
		...
		int getSize(int i)
		{
			return info[i].internal.size();
		}

		void resizeDR(int i, int size)
		{
			info[i].dRs.resize(size);
		}

		int getInteraction(int i, int j)
		{
			return info[i].internal[j].atom2;
		}

		Vector3D getDiff(int i, int j)
		{
			return info[i].internal[j].diff;
		}

		Real getDist(int i, int j)
		{
			return info[i].internal[j].dist;
		}

		Real getDR(int i, int j)
		{
			return info[i].dRs[j];
		}
