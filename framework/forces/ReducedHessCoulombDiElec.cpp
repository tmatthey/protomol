...
namespace ProtoMol
{
	//_________________________________________________________________ ReducedHessCoulombDielec
	Matrix3by3 ReducedHessCoulombDielec::operator()(const Real rawEnergy,
	                                                const Real rawForce,
	                                                Real a,
	                                                Real /*rDistSquared*/,
	                                                const Vector3D& rij,
	...
	{
		//std::cout<<"Inside CoulombDiElec "<<std::endl;
		...
		Real expsr = exp(-S * na);
		Real epsilon_r = D - (D - epsi) * 0.5 * expsr * (S * S * na * na + 2 * S * na + 2);
		Real k_r = na * epsilon_r;
		Real z_r = D - (D - epsi) * 0.5 * expsr * (S * S * na * na + 2 * S * na + 2 - S * S * S * na * na * na);
		Real tm1 = (coulombScalingFactor * scaledCharge_i * scaledCharge_j);///(k_r*k_r);

		Real Dervz_rVsr = (D - epsi) * 0.5 * expsr * (4 * S * S * S * a - S * S * S * S * a * na);

		Real P1r = -(z_r / na) / (k_r * k_r);

		Real P2r = (-1) * (k_r * Dervz_rVsr - 2 * z_r * z_r) / (k_r * k_r * k_r) / na - P1r / a;
