#ifndef EVALUATEBORN_H
#define EVALUATEBORN_H
...
void evaluateBorn(TOneAtomPair& myOneAtomPair,
  ...
  for (unsigned int i = 0; i<topo->atoms.size(); i++) {
    ...
    //std::cout<<std::setw(16)<<std::setprecision(10)<<"Atom "<<i<<" sasaFrac "<<-f<<std::endl; 
    ...
    topo->atoms[i].mySCPISM->bornRadius = 
      rw + gamma - gamma*dR_vdw2*(A_i + f) + topo->atoms[i].mySCPISM->polarFrac;
    //**********************************************
    //std::cout<<std::setw(16)<<std::setprecision(10)<<"Atom "<<i<<" born "<<topo->atoms[i].mySCPISM->bornRadius<<std::endl; 
  ...
  for (unsigned int i = 0; i<topo->atoms.size(); i++) {
    ...
    Real K = (D-1.0)/2;
    //Real sqrt_alpha_i = topo->atomTypes[topo->atoms[i].type].mySCPISM->sqrt_alpha;
	...
	//std::cout<<std::setprecision(12)<<"Atom "<<i<<" DsRb "<<Ds_r<<" dDsR "<<dDs_r<<std::endl;
    Real q_i = topo->atoms[i].scaledCharge;
    
    
    for (int k = 0; k < myOneAtomPair.getNonbondedForceFunction()->getSize(i); k++) {
      ...
      // I term
      Real forceval = 0.5*q_i*q_i*rBornRadius*rBornRadius*(1.0 - rDs_r - bornRadius*rDs_r*rDs_r*dDs_r)*myOneAtomPair.getNonbondedForceFunction()->getDR(i,j);
      
      // J term
      Real bornRadius_j = topo->atoms[j].mySCPISM->bornRadius;
      Real rBornRadius_j = 1.0 / bornRadius_j;
      //Real sqrt_alpha_j = topo->atomTypes[topo->atoms[j].type].mySCPISM->sqrt_alpha;
      ...
      forceval += 0.5*q_j*q_j*rBornRadius_j*rBornRadius_j*(1.0 - rDs_r_j - bornRadius_j*rDs_r_j*rDs_r_j*dDs_r_j)*myOneAtomPair.getNonbondedForceFunction()->getDR(j,i);
    ...
    //**********************************************
    // Eq. 4
    (*energies)[ScalarStructure::COULOMB] += 0.5*q_i*q_i*rBornRadius*(rDs_r-1);
    //**********************************************
    //selfe += 0.5*q_i*q_i*rBornRadius*(rDs_r-1);
