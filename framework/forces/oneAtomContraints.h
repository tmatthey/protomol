...
namespace ProtoMol {
  ...
  */
  struct HBondConstraint {
    ...
    static bool check(const GenericTopology* topo, int i, int j){return (!equal(topo->atoms[i].scaledMass,topo->atoms[j].scaledMass));}
  ...
  */
  struct SameMoleculeConstraint {
    enum {PRE_CHECK=1, POST_CHECK=0};
    static bool check(const GenericTopology* topo, int i, int j){return (topo->atoms[i].molecule == topo->atoms[j].molecule);}
    static std::string getPrefixId() {return "";}
