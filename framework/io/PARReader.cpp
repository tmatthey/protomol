...
namespace ProtoMol
{
	...
	PARReader::PARReader(PAR::CharmmTypeEnum charmmType):
		Reader(),
		myPAR(NULL), myCharmmType(charmmType), myCharmmTypeDetected(PAR::UNDEFINED)
	{
	}

	PARReader::PARReader(const std::string& filename, PAR::CharmmTypeEnum charmmType):
		Reader(filename),
		myPAR(NULL), myCharmmType(charmmType), myCharmmTypeDetected(PAR::UNDEFINED)
	{
	}
	...
	bool PARReader::open(const std::string& filename, PAR::CharmmTypeEnum charmmType)
	{
	...
	bool PARReader::read(PAR& par)
	{
		...
		bool isCharmm19 = (myCharmmType == PAR::UNDEFINED ? charmm19 > charmm28 : (myCharmmType == PAR::CHARMM19));
		...
		for (vector<vector<string>>::const_iterator i = input.begin(); i != input.end(); ++i , ++sig , ++line)
		{
			...
			case BOND:
				{
					if (s == "wwdd")
						par.bonds.push_back(PAR::Bond(par.bonds.size() + 1, j[0], j[1], toReal(j[2]), toReal(j[3])));
					else
			...
			case ANGLE:
				{
					if (s == "wwwdd")
						par.angles.push_back(PAR::Angle(par.angles.size() + 1, j[0], j[1], j[2], toReal(j[3]), toReal(j[4]), false, 0.0, 0.0));
					else if (s == "wwwdddd")
						par.angles.push_back(PAR::Angle(par.angles.size() + 1, j[0], j[1], j[2], toReal(j[3]), toReal(j[4]), true, toReal(j[5]), toReal(j[6])));
					else if (s == "wwwddwdd")
						par.angles.push_back(PAR::Angle(par.angles.size() + 1, j[0], j[1], j[2], toReal(j[3]), toReal(j[4]), true, toReal(j[6]), toReal(j[7])));
					else
			...
			case DIHEDRAL:
				{
					if (s == "wwwwddd")
					{
						...
						{
							par.dihedrals.push_back(PAR::Dihedral(par.dihedrals.size() + 1, j[0], j[1], j[2], j[3], toReal(j[4]), toInt(j[5]), toReal(j[6])));
						}
					}
					else if (s == "wwwwwdddd")
					{
						// Charmm19 with multiplicity > 1        
						// Add the dihedral ...
						PAR::Dihedral dihedral(par.dihedrals.size() + 1, j[0], j[1], j[2], j[3], toReal(j[6]), toInt(j[7]), toReal(j[8]));
			...
			case IMPROPER:
				{
					if (s == "wwwwddd")
						par.impropers.push_back(PAR::Improper(par.impropers.size() + 1, j[0], j[1], j[2], j[3], toReal(j[4]), toInt(j[5]), toReal(j[6])));
					else
			...
			case NONBONDED:
				{
					if ((!isCharmm19 && s != "wdddddd" && s != "wddd") || (isCharmm19 && s != "wdddd"))
					{
					...
					nonbonded.sigma = toReal(j[2 + (isCharmm19 ? 0 : 1)]) * (isCharmm19 ? PAR::Nonbonded::SIGMA_CHARMM19_TO_CHARMM28 : 1.0);
					nonbonded.polarizability2 = (s.size() >= 7 ? toReal(j[4]) : 0.0);
					nonbonded.epsilon14 = (s.size() >= 5 ? toReal(j[3 + (isCharmm19 ? 0 : 2)]) : nonbonded.epsilon);
					nonbonded.sigma14 = (s.size() >= 5 ? toReal(j[4 + (isCharmm19 ? 0 : 2)]) : nonbonded.sigma) * (isCharmm19 ? PAR::Nonbonded::SIGMA_CHARMM19_TO_CHARMM28 : 1.0);
			...
			case NBFIX:
				{
					if (s == "wwdddd")
						par.nbfixs.push_back(PAR::Nbfix(par.nbfixs.size() + 1, j[0], j[1], toReal(j[2]), toReal(j[3]), toReal(j[4]), toReal(j[5])));
					else if (s == "wwdd")
						par.nbfixs.push_back(PAR::Nbfix(par.nbfixs.size() + 1, j[0], j[1], toReal(j[2]), toReal(j[3]), toReal(j[2]), toReal(j[3])));
					else
			...
			case HBOND:
				{
					if (s == "wwdd")
						par.hbonds.push_back(PAR::Hbond(par.hbonds.size() + 1, j[0], j[1], toReal(j[2]), toReal(j[3])));
					else
