...
//       report << "Unit cell   :"<<i32<<endr;
      ...
      File::read(reinterpret_cast<char*>(&freeIndexes),sizeof(int32)); if(mySwapEndian) swapBytes(freeIndexes);

      // Skip header
      myFile.seekg (84+4, std::ios::beg);
      n = 0; File::read(reinterpret_cast<char*>(&n),sizeof(int32)); if(mySwapEndian) swapBytes(n);
      if(n != 84 || myFile.fail()){
	myFile.setstate(std::ios::failbit);
	close();
	return false;
      ...
      n = 0; 
      m = 0; 
      File::read(reinterpret_cast<char*>(&n),sizeof(int32)); if(mySwapEndian) swapBytes(n);
      File::read(reinterpret_cast<char*>(&m),sizeof(int32)); if(mySwapEndian) swapBytes(m);
      ...
      File::read(reinterpret_cast<char*>(&l),sizeof(int32)); if(mySwapEndian) swapBytes(l);
	...
	return false;
      ...
      l = 0;
      int32 count = 0; 
      File::read(reinterpret_cast<char*>(&n),sizeof(int32)); if(mySwapEndian) swapBytes(n); // 4
      File::read(reinterpret_cast<char*>(&count),sizeof(int32)); if(mySwapEndian) swapBytes(count); // number of atoms
      File::read(reinterpret_cast<char*>(&l),sizeof(int32)); if(mySwapEndian) swapBytes(l); // 4
    ...
    count = 0;
    File::read(reinterpret_cast<char*>(&count),sizeof(int32)); if(mySwapEndian) swapBytes(count); 
    count /= sizeof(int32);
    if((unsigned int)count != myX.size() || myFile.fail()){
      myFile.setstate(std::ios::failbit);
      close();
