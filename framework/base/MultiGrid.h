...
namespace ProtoMol
{
	...
	public:
		...
		{
			begin = myV[level].begin();
			end = myV[level].end();
		}

		void getQ(int level, Real*& begin, Real*& end)
		{
			begin = myQ[level].begin();
			end = myQ[level].end();
		}
	...
	{
		Real x = (coord.x - myMin.x) * myHXr;
		Real y = (coord.y - myMin.y) * myHYr;
		Real z = (coord.z - myMin.z) * myHZr;
	                                                                      ...
	                                                                      Vector3D h, Vector3D origin)
	{
		...
		if ((!pbcX && h.x <= 0.0) || (!pbcY && h.y <= 0.0) || (!pbcZ && h.z <= 0.0))
		{
			Report::report << Report::error << "[MultiGrid::initialize]";
			if (pbcX) Report::report << " h.x (=" << h.x << ") must be > 0.";
			if (pbcY) Report::report << " h.y (=" << h.y << ") must be > 0.";
			if (pbcZ) Report::report << " h.z (=" << h.z << ") must be > 0.";
			Report::report << Report::endr;
		}
	...
	void MultiGrid<TInterpolation, TKernel, pbcX, pbcY, pbcZ>::updateSize(Vector3D min, Vector3D max)
	{
		...
		if (!pbcX)
		{
			min.x -= myH.x * BORDER_TOLERANCE;
			max.x += myH.x * BORDER_TOLERANCE;
			nx1 = (int)(ceil((max.x - myOrigin.x) / myH.x) - floor((min.x - myOrigin.x) / myH.x) + b + 1);
			nx = nx1 + BORDER;
		}
		if (!pbcY)
		{
			min.y -= myH.y * BORDER_TOLERANCE;
			max.y += myH.y * BORDER_TOLERANCE;
			ny1 = (int)(ceil((max.y - myOrigin.y) / myH.y) - floor((min.y - myOrigin.y) / myH.y) + b + 1);
			ny = ny1 + BORDER;
		}
		if (!pbcZ)
		{
			min.z -= myH.z * BORDER_TOLERANCE;
			max.z += myH.z * BORDER_TOLERANCE;
			nz1 = (int)(ceil((max.z - myOrigin.z) / myH.z) - floor((min.z - myOrigin.z) / myH.z) + b + 1);
			nz = nz1 + BORDER;
		}
		...
		if (!pbcX && !pbcY && !pbcZ)
		{
			if (min.x > myCMin.x && min.y > myCMin.y && min.z > myCMin.z &&
				max.x < myCMax.x && max.y < myCMax.y && max.z < myCMax.z &&
			...
			fabs((float)(nz - myDim[myLevels - 1].z)) > BORDER)
		{
			...
			if (pbcX)
				myHX = (max.x - min.x) / ((Real)myNX);
			else
				myHX = myH.x;
			if (pbcY)
				myHY = (max.y - min.y) / ((Real)myNY);
			else
				myHY = myH.y;
			if (pbcZ)
				myHZ = (max.z - min.z) / ((Real)myNZ);
			else
				myHZ = myH.z;
		...
		if (!pbcX)
		{
			min.x = floor((min.x - myOrigin.x) / myH.x - (myNX - nx1) / 2) * myH.x + myOrigin.x;
			max.x = min.x + (myNX - 1.0 - b) * myH.x;
			myCMin.x = min.x;
			myCMax.x = max.x;
			min.x -= myH.x * (b / 2);
			max.x += myH.x * (b - b / 2);
		...
		if (!pbcY)
		{
			min.y = floor((min.y - myOrigin.y) / myH.y - (myNY - ny1) / 2) * myH.y + myOrigin.y;
			max.y = min.y + (myNY - 1.0 - b) * myH.y;
			myCMin.y = min.y;
			myCMax.y = max.y;
			min.y -= myH.y * (b / 2);
			max.y += myH.y * (b - b / 2);
		...
		if (!pbcZ)
		{
			min.z = floor((min.z - myOrigin.z) / myH.z - (myNZ - nz1) / 2) * myH.z + myOrigin.z;
			max.z = min.z + (myNZ - 1.0 - b) * myH.z;
			myCMin.z = min.z;
			myCMax.z = max.z;
			min.z -= myH.z * (b / 2);
			max.z += myH.z * (b - b / 2);
	...
	inline void MultiGrid<TInterpolation, TKernel, pbcX, pbcY, pbcZ>::clear()
	{
		for (int level = 0; level < myLevels; level++)
		{
			//     Array<Real,3>& rV = myV[level];
			//     Array<Real,3>& rQ = myQ[level];
			//     for (int i = 0; i < myDim[level].x+(pbcX?0:mySigma); i++){
			//       for (int j = 0 ; j < myDim[level].y+(pbcY?0:mySigma); j++){
			// 	for (int k = 0; k < myDim[level].z+(pbcZ?0:mySigma); k++){
			// 	  rV[i][j][k] = 0.0;
			// 	  rQ[i][j][k] = 0.0;
			// 	}
			//       }
