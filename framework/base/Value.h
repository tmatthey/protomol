	...
	class Value
	{
		...
		*/
		...
		public:
			...
			{
				bool tmp = ok;
				holder = ValueTrait::init();
				ok = ValueTrait::check(Constraint(), holder);
				return tmp;
			}

			virtual unsigned int size() const
			{
				return ValueTrait::size(holder);
			}

			virtual void read(std::istream& is)
			{
				ok = ValueTrait::read(is, holder) && ValueTrait::check(Constraint(), holder);
			}
			...
			{
				return ConstraintValueType::getString((ConstraintValueType::Enum)Constraint::value);
			}
