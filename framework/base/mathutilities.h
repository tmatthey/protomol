...
namespace ProtoMol
{
  ...
  template<int _power> inline Real power(Real x){
	...
	template <>
	inline Real power<0>(Real)
	{
		return 1;
	}

	template <>
	inline Real power<1>(Real x)
	{
		return x;
	}
	...
	namespace Private
	{
		template <bool cmp, int A>
		struct BinaryExponentHelper
		{
			enum
			{
				N=1
		...
		template <int A>
		struct BinaryExponentHelper<false, A>
		{
			enum
			{
				N = 1 + BinaryExponentHelper<A <= 1, A / 2>::N
			};
		};
	}
	 ...
	 */
	template <int P>
	struct BinaryExponent
	{
		enum
		{
			N=Private::BinaryExponentHelper<P <= 1, P / 2>::N
		};
	};
