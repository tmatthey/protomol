...
namespace ProtoMol
{
	//_________________________________________________________________ LookUpTableBase
	/**
	 * Generic look up table covering the range [2^n0 <= from, to < 2^n1] with look up (hashed) 
	 * values defined by TLookUpValue and digit/bit precision PRE. lookUpExp is the exponent of 
	 * the hash value, yields also for the range passed to the constructor, e.g., if you like 
	 * to use a look up table for the squared norm, then set lookUpExp to 2. LookUpTableBase
	 * does also support switching functions. LookUpTableBase is the base class of any tabulated 
	 * potential (e.g. CoulombForce). TLookUpValue implements the function to be hashed with 
	...
	public:
		/// number of values per entry (0: value, 2: gradient, 2: 2. interp., 3: 3. interp.)
		enum
		{
			NV=4
		};
		...
		enum
		{
			E=BinaryExponent<std::numeric_limits<TReal>::max_exponent - std::numeric_limits<TReal>::min_exponent>::N
		};

		/// entries per look up up
		enum
		{
			STEP=TLookUpValue::ENTRIES * NV
		};
		...
		enum
		{
			M=(sizeof(TReal) * 8 - 1 - E)
		};

		/// look up mask for hashing
		enum
		{
			SHIFT=(sizeof(TReal) * 8 - 1 - E - PRE)
		};
	...
	public:
		...
		LookUpTableBase(const LookUpTableBase& rhs)
		{
			if (&rhs != this)
			{
				mySize = rhs.mySize;
		...
		LookUpTableBase(TReal from,
		                TReal to,
		                int lookUpExp,
		                const TLookUpValue& lvf,
		                const TSwitchingFunction& swf,
		                unsigned int align = 128): myTable(NULL),
		                                           myFirst(absindex(lowerboundExp2(from))),
		                                           myLast(((expn(to) + 1 - expn(from)) * N) * STEP),
		                                           mySize(((expn(to) + 1 - expn(from)) * N + 1) * STEP),
		                                           myAlign(align),
		                                           myMem(new TReal[((expn(to) + 1 - expn(from)) * N + 1) * STEP + align / sizeof(TReal)])
		{
			// align table
			myTable = myMem;
			...
			for (int i = 0; i < mySize / STEP; i++)
			{
				...
				for (unsigned int j = 0; j < static_cast<unsigned int>(STEP); j += NV)
				{
				...
				if (i > 0)
				{
					...
					for (unsigned int j = 0; j < static_cast<unsigned int>(STEP); j += NV)
					{
						...
						t[2 + j] = c;
						t[3 + j] = d;
						Real ev = fabs(((t[3 + j] * x + t[2 + j]) * x + t[1 + j]) * x + t[0 + j] - t[0 + STEP + j]);
						Real eg = fabs((3.0 * t[3 + j] * x + 2.0 * t[2 + j]) * x + t[1 + j] - t[1 + STEP + j]);
						errv = (errv < ev ? ev : errv);
						errg = (errg < eg ? eg : errg);
					}
				}
			}
			Report::report << Report::debug(2) << "Number of interpolation points:" << mySize / STEP << "," << (long)(myTable - myMem);
		...
		LookUpTableBase& operator=(LookUpTableBase const& rhs)
		{
			if (&rhs != this)
			{
				mySize = rhs.mySize;
		...
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// New methods of class LookUpTableBase
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	protected:
		...
		void interpolate(Real a, Real b, Real c, Real d, Real dt, Real& energy, Real& force) const
		{
	...
	template <class TLookUpValue, unsigned int PRE, typename TReal>
	const typename LookUpTableBase<TLookUpValue, PRE, TReal>::TInt LookUpTableBase<TLookUpValue, PRE, TReal>::myMask = ~((1LL << SHIFT) - 1LL);
}
