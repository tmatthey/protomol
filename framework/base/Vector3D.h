...
namespace ProtoMol {
  ...
  public:
    ...
    Real operator[](int index) const{
      if(index < 0 || index > 2)
	Report::report << Report::error << "[Vector3D::operator[] const] index out of range"<<Report::endr;
    ...
    Vector3D add(const Vector3D& b) const {return (*this)+b;}
  
    Vector3D operator-(const Vector3D& b) const  {
      return Vector3D(x-b.x,y-b.y,z-b.z);
    }
    Vector3D subtract(const Vector3D& b) const {return (*this)-b;}
    ...
    /// dot product
    Real dot(const Vector3D& b) const { return (*this)*b;}
    ...
    /// cross product
    Vector3D cross(const Vector3D& b) const  {return (*this)^b;}


    Vector3D operator*(Real w) const  {
      return Vector3D(x*w,y*w,z*w);
    }
    Vector3D multiply(Real w) const {return (*this)*w;}
  
    Vector3D operator/(Real w) const  {
      return Vector3D(x/w,y/w,z/w);
    }
    Vector3D divide(Real w) const {return (*this)/w;}
    ...
    Vector3D &intoAdd(const Vector3D& b) {return (*this)+=b;}
    ...
    Vector3D &intoSubtract(const Vector3D& b) {return (*this)-=b;}
    ...
    Vector3D &intoMultiply(Real w) {return (*this)*=w;}
    ...
    Vector3D &intoDivide(Real w) {return (*this)/=w;}
    ...
    Vector3D normalized() const {
      ...
      if (len == 0.0) {
	Report::report << Report::recoverable << "[Vector3D::normalized] length is zero."<<Report::endr;
    ...
    friend Report::MyStreamer& operator<< (Report::MyStreamer& OS, const Vector3D & coords ) {
      OS << "("<<coords.x<<","<<coords.y<<","<<coords.z<<")";
      return OS;
    }
