...
namespace ProtoMol
{
	...
	public:
		...
		{
			return myTable[static_cast<int>(i)];
		}
	...
	{
		return ((myTable[VIRIALXX] + myTable[VIRIALYY] + myTable[VIRIALZZ]) / 3.0 / volume * Constant::PRESSUREFACTOR);
	}

	inline Real ScalarStructure::molecularPressure(Real volume) const
	{
		return ((myTable[MOLVIRIALXX] + myTable[MOLVIRIALYY] + myTable[MOLVIRIALZZ]) / 3.0 / volume * Constant::PRESSUREFACTOR);
	}
}
