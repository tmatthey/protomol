...
namespace ProtoMol
{
	...
	public:
		...
		template <typename T>
		Parameter(const std::string& k, const Value& val, T def): keyword(k), value(val), defaultValue(val)
		{
		...
		template <typename T>
		Parameter(const char* k, const Value& val, T def): keyword(std::string(k)), value(val), defaultValue(val)
		{
			defaultValue.set(def);
		}
		...
		template <typename T>
		Parameter(const std::string& k, const Value& val, T def, const Text& t): keyword(k), value(val), defaultValue(val), text(t.text)
		{
			defaultValue.set(def);
		}
		...
		template <typename T>
		Parameter(const char* k, const Value& val, T def, const Text& t): keyword(std::string(k)), value(val), defaultValue(val), text(t.text)
		{
			defaultValue.set(def);
		}
