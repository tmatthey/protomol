...
namespace ProtoMol
{
	...
	Real randomGaussianNumber(unsigned int seed)
	{
		...
		if (!iset)
		{ // we do not have an extra result ready, 
	...
	int getTimerSeed()
	{
		...
		return static_cast<int>((currentTime * 10000 - floor(currentTime * 10000)) * 100000);
	}
	...
	void splitRangeQuadratic(unsigned int p, unsigned int from, unsigned int to,
	                         vector<PairUInt>& fromRange, vector<PairUInt>& toRange)
	{
		fromRange.clear();
		toRange.clear();
		...
		if (p > 2)
		{
			...
			{
				fromRange.push_back(PairUInt(static_cast<unsigned int>(from + (size * i) / p), static_cast<unsigned int>(from + (size * (i + 1.0)) / p)));
				toRange.push_back(PairUInt(static_cast<unsigned int>(from + (size * i) / p), static_cast<unsigned int>(from + (size * (i + 1.0)) / p)));
			}
			for (unsigned int i = 0; i < p; i++)
			{
				...
				{
					fromRange.push_back(PairUInt(static_cast<unsigned int>(from + (size * i) / p), static_cast<unsigned int>(from + (size * (i + 1.0)) / p)));
					toRange.push_back(PairUInt(static_cast<unsigned int>(from + (size * j) / p), static_cast<unsigned int>(from + (size * (j + 1.0)) / p)));
				}
			}
		}
		else
	                    ...
	                    vector<PairUInt>& fromRange, vector<PairUInt>& toRange)
	{
		...
		for (unsigned int i = 0; i < p; i++)
		{
			second = static_cast<int>(size - 0.5 * sqrt(power<2>(1.0 + size * 2.0) - (4.0 * (i + 1.0) * size * (size + 1.0)) / p) + 0.5);
