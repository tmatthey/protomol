...
namespace ProtoMol
{
	...
	OutputFile::OutputFile(): Output(), myFilename(""), myCacheFreq(0), myCount(0), myCacheSize(0), myCloseTime(0.0)
	{
	}
	...
	OutputFile::OutputFile(const string& filename,
	                       ...
	                       int cacheFreq,
	                       int cacheSize,
	                       Real closeTime): Output(freq),
	                                        myFilename(filename),
	                                        myCacheFreq(cacheFreq),
	                                        myCount(0),
	...
	void OutputFile::flushCache()
	{
		...
		{
			if (!reopen())
				report << error << " Can not open \'" << myFilename << "\' for " << this->getId() << "." << endr;
			myFile << myBuffer.str();
			reclose();
		}
		doFlushCache();
	...
	void OutputFile::getParameters(vector<Parameter>& parameter) const
	{
		...
		parameter.push_back(Parameter(getId() + string("OutputFreq"), Value(myOutputFreq, ConstraintValueType::Positive()), Text("output frequency")));
		parameter.push_back(Parameter(getId() + string("CacheFreq"), Value(myCacheFreq, ConstraintValueType::Positive()), 1, Text("frequency to flush the cache to file")));
		parameter.push_back(Parameter(getId() + string("CacheSize"), Value(myCacheSize, ConstraintValueType::NotNegative()), 0, Text("minimal size of cached data to flush to file")));
		parameter.push_back(Parameter(getId() + string("CloseTime"), Value(myCloseTime, ConstraintValueType::NotNegative()), 1.0, Text("minimal time interval between two writes to close the file temporarily")));
	}

	bool OutputFile::adjustWithDefaultParameters(vector<Value>& values, const Configuration* config) const
	{
