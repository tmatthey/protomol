...
static void doEvenSwitch(int step, GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Integrator *integrator, ScalarStructure *scalar, Real *numSwitches, Real *historyArray, int histSize, Real nodeTemp,vector<Real> &countExch);
static void doOddSwitch(int step, GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Integrator *integrator, ScalarStructure *scalar, Real *numSwitches, Real *historyArray, int histSize, Real nodeTemp,vector<Real> &countExch);
...
static void doSwitch(GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Real myTemp, int address, Real *historyArray, int histSize);
...
int main(int argc, char **argv) {
  ...
  countExchanges.resize(Parallel::getNum());
  for (int i = 0; i < Parallel::getNum(); i++)
  {
      countExchanges[i] = 0.0;
      //std::cout << "Initialize countExchanges for " << Parallel::getId() << " :" << countExchanges[i] << endl;
  }
  ...
  for (int i = 0; i < numTotalSwitches; i++) {
	...
	if  ((Parallel::getId() % 2) == 0)
	  doEvenSwitch(i, topo, &positions, &velocities, integrator, &scalar, &numSwitches, tempHistory, numTotalSwitches + 1,temp,countExchanges); // used when this node should compute delta
	else
	  doOddSwitch(i, topo, &positions, &velocities, integrator, &scalar, &numSwitches, tempHistory, numTotalSwitches + 1,temp,countExchanges); // used when the node should not compute delta
    }
    Parallel::sync(); // sync up
  }
...
void doEvenSwitch(int step, GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Integrator *integrator, ScalarStructure *scalar, Real *numSwitches, Real *historyArray, int histSize, Real Nodetemp, vector<Real> &countExch) {
  int address = getNextAddress(step); // returns the appropriate address based on the step and node ID
  //Real myTemp = temperature(topo, velocities); temperature function incorrect based on # degrees of freedom
  Real myTemp = Nodetemp;  
  ...
  if (canDoEvenSwitch(address, myTemp, myEnergy)) { // check to see if a switch is possible
    doSwitch(topo, positions, velocities, myTemp, address, historyArray, histSize); // do the switch
    Parallel::isolateNode(); // hide MPI to reinitialize the stats (e.g. potential energy, etc.)
    integrator->initialize(topo, positions, velocities, scalar);
    Parallel::integrateNode();
    //std::cout << "countExch: " << countExch[address] << endl;
...
void doOddSwitch(int step, GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Integrator *integrator, ScalarStructure *scalar, Real *numSwitches, Real *historyArray, int histSize, Real Nodetemp, vector<Real> &countExch) {
  ...
  if (canDoOddSwitch(address, myTemp, myEnergy)) { //check to see if a switch is needed
    doSwitch(topo, positions, velocities, myTemp, address, historyArray, histSize); //switch
...
void doSwitch(GenericTopology *topo, Vector3DBlock *positions, Vector3DBlock *velocities, Real myTemp, int address, Real *historyArray, int histSize) {
  Real otherTemp = 0;
  int redrawVelocities = 1;
  TimerStatistic::timer[TimerStatistic::COMMUNICATION].start();
  // swap temps
  ...
  TimerStatistic::timer[TimerStatistic::COMMUNICATION].stop();
  if (redrawVelocities == 1)
    {
      int mySeed = getTimerSeed();
      randomVelocity(myTemp,topo,velocities,mySeed);
	...
	(*velocities)[j] = (*velocities)[j] * factor;
    }
  // rescale velocities from otherTemp to myTemp
}
...
int getNextAddress(int step) {
  ...
  if ((result % 2) == 0)
    result = ((step % 2) == 0 ? result + 1 : result - 1);
  else
    result = ((step % 2) == 0 ? result - 1 : result + 1);
  return result;
}
