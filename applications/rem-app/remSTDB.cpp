...
static void doSwitch(Vector3DBlock *positions, Vector3DBlock *velocities, Real myTemp,
                     int address, Real *historyArray, int histSize);
...
int main(int argc, char **argv) {
  ...
  Real myEnergy = 0;
  int stepLength = 0;
  int Reverse = 0; // added to allow Sequential REM reversal to meet DB
  ...
  for (int i = 0; i < numTotalSwitches; i++) {
	...
	if  (Parallel::getId() == Parallel::getMasterId()) // MasterNode
	{
	  ...
	  { 
            MTPattern = (patVec.size()-1) - (i % patVec.size());
            std::cout << "MTPattern: " << MTPattern << endl;
	  }
	...
	for (int i=0; i < NumReplicas; i++)
	{
	  if (Parallel::getId() == switchPairs[i])
	  {
	    if ((i % 2) == 0)
	    {
	      ...
	      doSwitch(&positions, &velocities, myTemp, int(switchPairs[i+1]), tempHistory, numTotalSwitches + 1);
