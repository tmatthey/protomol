...
int main(int argc, char **argv) {
  ...
  report << debug(5)<< "tempString "<<tempString<<endr;
  ...
  // numPairs : no of possible pairs...for 4 replicas (1,2),(1,3),(1,4),(2,3),(2,4),(3,4)= 6.
  ...
  for (int i = 0; i < numTotalSwitches; i++) {
	...
	myEnergy = scalar.potentialEnergy();
        ...
        //gathers temp data from all process into process of rank 0
	...
	// START BSI LOGIC ***************************
        ...
        if (Parallel::getId() == Parallel::getMasterId()) 
	  {
	    ...
	    calcExchMat2DNorm(exchMat2D, exchMat3D, pairs);
	    for(int i=0;i<exchMat2DSum.size();i++)
	      {
		for(int j=0;j<exchMat2DSum[i].size();j++)
		  exchMat2DSum[i][j]=exchMat2DSum[i][j] + exchMat2D[i][j];
	      }
	    vector < Real > runSum;
	    runSum.resize(numPairs + 1);

	    runSum[0] = exchMat2D[1][0];
	    int count = 1;
	    for (int i=2; i < numReplicas; i++)
	      {
		for (int j=0; j < i; j++)
		  {
		    runSum[count] = exchMat2D[i][j] + runSum[count - 1];
		    count = count + 1;
		  }
	      }
	    ...
	    count = 0;
	    Real myRand = drand48();
	    while(myRand > runSum[count])
	      count++;
	    if (count < numPairs)
	      {
		switchPair[0] = pairs[count][0];
		switchPair[1] = pairs[count][1];
	      }
	    else
	      {
...
string changeTempDir(string temperature, Configuration &config) {
  ...
  report << debug(5) <<"new dir "<<new_dir<<endr;
  if (retval == -1)
