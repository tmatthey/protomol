...
declareInputValue(InputSTAGE,STRING,NOTEMPTY);
declareInputValue(InputReportFreq,BOOL,NOCONSTRAINTS);
declareInputValue(InputTICalc,BOOL,NOCONSTRAINTS);
declareInputValue(InputCoordFreq,INT,NOTNEGATIVE);

defineInputValue(InputXSC,"xscfile");
defineInputValue(InputSTAGE,"stagefile");
defineInputValue(InputReportFreq,"reportOnFreq");
defineInputValue(InputTICalc,"TICalc");
defineInputValue(InputCoordFreq,"posfilefreq");


//_____________________________________________________________________ protomol for oSGMD
//  oSGMD -- Osmotic SemiGrand ensemble Molecular Dynamics
...
int main(int argc, char **argv) {
  ...
  InputSTAGE::registerConfiguration(&config);
  ...
  InputCoordFreq::registerConfiguration(&config,1);

  //
  // Configure OutputFactory
  //
  oSGregisterOutputExemplars();
  ...
  //
  // Parse command line
  vector<vector<string> > args = parseCommandLine(argc,argv,&config,oSGregisterForceExemplars);
  ...
  // STAGE
  //
  // create the STAGE reader object
  // The STAGE file contains the details of how we are to perform the molecular insertions/deletions
  // in stages.
  STAGEReader stageReader;

  if(!stageReader.open(config[InputSTAGE::keyword]))
    report << error << "Can't open STAGE file \'"<<config[InputSTAGE::keyword]<<"\'."<<endr;

  // create the storage object for the STAGE information
  STAGE stage;

  // read in the STAGE file information
  if(!(stageReader >> stage))
    report << error << "Could not parse STAGE file \'"<<config[InputSTAGE::keyword]<<"\'."<<endr;
  report << plain << "Using STAGE file \'"<<config[InputSTAGE::keyword]<<"\'." << endr;

  //
  // PAR
  //
  // create the PAR reader for oSGMD simulations
  // here, unlike for iSGMD, we use the standard PAR reader
  PARReader parReader;
  ...
  // Build topology
  buildOSGTopology(topo,psf,par,config[InputDihedralMultPSF::keyword]);
  ...
  //
  oSGregisterForceExemplars(topo);
  ...
  // OSG integrator functions
  oSGIntegrator* osgIntegrator;
    
  if (integrator->getIdNoAlias() == "muVTVerlet" ) 
    osgIntegrator = dynamic_cast<muVTIntegrator*>( integrator->bottom() );
  else if (integrator->getIdNoAlias() == "NfPTVerlet")
    osgIntegrator = dynamic_cast<NfPTIntegrator*>( integrator->bottom() );
  else report << "Integrator " << integrator->getIdNoAlias() << " is not currently supported in oSGProtomol" << endr;

  // store the detailed molecule insertion/deletion information 
  osgIntegrator->indexTypes(topo, stage, par, config[InputDihedralMultPSF::keyword], seed);
  osgIntegrator->setTrajectoryFreq((int)config[InputCoordFreq::keyword]);

  // XSC (eXtended System Coordinates)
  // read this only if an XSC file is specified in the config file.
  if(config[InputXSC::keyword].valid()) osgIntegrator->readXSCs(config[InputXSC::keyword],topo);
  ...
  while(step < last) {
    ...
    // integrate the equations of motion for this timestep
  ...
  if((bool)config[InputTICalc::keyword]) {
    report.precision(8);
    report << plain << "d(E) / d(lambda) = " << osgIntegrator->getAveDeltaMu(myNumSteps) << endr;
  }
  // if we are doing a standard oSGMD simulation then report the average chemostat temperature
  else {
    report.precision(6);
    Real AverageLambdaT = osgIntegrator->getAveLambdaT(myNumSteps);
