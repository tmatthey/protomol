/* -*- c++ -*- */
#ifndef ISGMTORSIONSYSTEMFORCE_H
#define ISGMTORSIONSYSTEMFORCE_H

#include "SystemForce.h"
#include "ScalarStructure.h"

namespace ProtoMol {
  //_________________________________________________________________ iSGMTorsionSystemForce
  template<class TBoundaryConditions>
  class iSGMTorsionSystemForce: public SystemForce {
  ...
  template<class TBoundaryConditions> 
  inline void iSGMTorsionSystemForce<TBoundaryConditions>::calcTorsion(const TBoundaryConditions &boundary,
    ...
    Vector3D r12(boundary.minimalDifference((*positions)[a2],(*positions)[a1]));  
    //Vector3D r23((*positions)[a2] - (*positions)[a3]);       // Vector from atom 2 to atom 3
    Vector3D r23(boundary.minimalDifference((*positions)[a3],(*positions)[a2]));   
    //Vector3D r34((*positions)[a3] - (*positions)[a4]);       // Vector from atom 3 to atom 4
    Vector3D r34(boundary.minimalDifference((*positions)[a4],(*positions)[a3]));      
	...
	Real ftorsion = currTorsion.periodicity[i]
	...
	// Add energy
	Real Vtorsion = currTorsion.forceConstant[i] * ( 1.0 + cos(
							     currTorsion.periodicity[i] * phi
							     + currTorsion.phaseShift[i] ) );
	energy += Vtorsion;

	// Add chemical potential difference
        if (currTorsion.DeltaK[i] != 0.0 && currTorsion.DeltaPhase[i] != 0.0) {
          Real DMUtorsion = currTorsion.DeltaK[i] * ( 1.0 + cos(currTorsion.periodicity[i] * phi + currTorsion.phaseShift[i]) )
            + ftorsion * currTorsion.DeltaPhase[i] / currTorsion.periodicity[i];
          deltaMu += DMUtorsion;
        }
        
//#define ISG_DEBUG_TORSION
#ifdef ISG_DEBUG_TORSION
    Report::report.precision(8);
    Report::report << "Torsion atoms: " << a1 << ", " << a2 << ", " << a3 << ", " << a4 << Report::endr;
    Report::report << "k = " << currTorsion.forceConstant[i]
		   <<  ", psi0 = " << currTorsion.phaseShift[i]
		   << ", n = " << currTorsion.periodicity[i]
		   << Report::endr;
    Report::report << "DeltaK = " << currTorsion.DeltaK[i]
		   <<  ", DeltaPsi0 = " << currTorsion.DeltaPhase[i]
		   << Report::endr;
    Report::report << "Torsion angle = " << phi << Report::endr;
    Report::report << "current energy (torsion) = " << energy << Report::endr;
    Report::report << "current DeltaMu (torsion) = " << deltaMu << Report::endr;
#endif
	...
	Real ftorsion = 2.0 * currTorsion.forceConstant[i] * diff;
	dpotdphi += ftorsion;

	// Add energy
	Real Vtorsion = currTorsion.forceConstant[i] * diff * diff;
	energy += Vtorsion;
	
	// Add chemical potential difference
        if (currTorsion.DeltaK[i] != 0.0 && currTorsion.DeltaPhase[i] != 0.0) {
          Real DMUtorsion = currTorsion.DeltaK[i] * diff * diff + ftorsion * currTorsion.DeltaPhase[i];
          deltaMu += DMUtorsion;
        }

#ifdef ISG_DEBUG_TORSION
    Report::report.precision(8);
    Report::report << "Improper atoms: " << a1 << ", " << a2 << ", " << a3 << ", " << a4 << Report::endr;
    Report::report << "k = " << currTorsion.forceConstant[i]
		   <<  ", psi0 = " << currTorsion.phaseShift[i]
		   << Report::endr;
    Report::report << "DeltaK = " << currTorsion.DeltaK[i]
		   <<  ", DeltaPsi0 = " << currTorsion.DeltaPhase[i]
		   << Report::endr;
    Report::report << "Improper angle = " << phi << Report::endr;
    Report::report << "current energy (improper) = " << energy << Report::endr;
    Report::report << "current DeltaMu (improper) = " << deltaMu << Report::endr;
#endif
  ...
  template<class TBoundaryConditions> 
  inline Real iSGMTorsionSystemForce<TBoundaryConditions>::calcTorsionEnergy(const TBoundaryConditions &boundary,
									     const Torsion& currTorsion,
									     const Vector3DBlock* positions){
    int a1 = currTorsion.atom1;
    int a2 = currTorsion.atom2;
...
#endif /* ISGMTORSIONSYSTEMFORCE_H */
